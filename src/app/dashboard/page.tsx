      ...
      >
        <div className="flex-1 min-w-0 mr-4">
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            <h2 className="font-bold text-lg text-white/90 truncate">
        ...
        {expanded && (
          ...
          >
            <div className="px-6 pb-6 border-t border-white/[0.06] pt-4 space-y-4">
              ...
              <button
                onClick={onNavigate}
                className="flex items-center gap-1.5 text-xs font-semibold text-violet-400 hover:text-violet-300 transition-colors mt-1"
              >
...
export default function Dashboard() {
  ...
  if (!user) {
    return (
      <div className="dark flex h-screen w-full items-center justify-center bg-[#080810]">
        <LoaderFour />
      </div>
    );
  }
  ...
  const Header = () => (
    <div className="flex justify-between items-center mb-8">
      <div>
        <h1 className="text-3xl font-bold text-white/90">Your Interviews</h1>
        <p className="text-white/40 text-sm mt-1">Welcome back, {user?.email}</p>
      </div>
  ...
  if (loading) {
    return (
      <main className="relative min-h-screen bg-[#080810] p-8 overflow-hidden">
        <MeshGradient />
        <div className="relative z-10 max-w-4xl mx-auto">
          <Header />
          <div className="grid gap-4">
            {[1, 2, 3].map((i) => (
              <div key={i} className="bg-white/[0.06] p-6 rounded-xl border border-white/[0.08] animate-pulse">
                <div className="h-6 w-3/4 bg-white/[0.06] rounded mb-4"></div>
                <div className="flex items-center gap-4">
                  <div className="h-4 w-32 bg-white/[0.06] rounded"></div>
        ...
        <div className="grid gap-4">
          ...
          {interviews.length === 0 && (
            ...
            >
              ...
              <p className="text-white/40 text-lg font-medium mb-2">No interviews yet</p>
              <p className="text-white/20 text-sm mb-6">Start your first interview to see results here</p>
              <button onClick={() => router.push("/")} className="btn-violet flex items-center gap-2">
                <Zap size={16} />
